...
import {NotificationOption, Permission} from './main/types'
...
const App: React.FC = () => {
  ...
  return (
    ...
    <View style={{flex: 1}}>
      ...
      <Appbar.Header>
        <Appbar.Content title="Deepak Kaligotla" />

        {Platform.OS === 'ios' && (
          <Appbar.Action
            icon="image-multiple"
            onPress={() => {
              RNPermissions.openPhotoPicker().catch((error: any) => {
                console.error(error);
              });
            }}
          />
      ...
      <ScrollView>
        {PERMISSIONS_VALUES.map((item: any, index: any) => {
